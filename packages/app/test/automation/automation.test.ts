...
describe('automationCurves', () => {
  it('gives each of the eight tracks its level curve', () => {
    const t = tracks((t) => (t[1]!.mute = pts([0, 0], [3, 1])));
    const curves = automationCurves(t);
    expect(curves).toHaveLength(8);
    expect(curves[1]).toEqual(levelCurve(t, 1));
  });
});
