...
const take = { id: 't1', title: '20260919 Sunday drift hoppp', durationSec: 1, recordedAt: '2026-09-21T12:00:00.000Z' } as HopSequence;
...
describe('exportTake', () => {
  it('asks where to save first, offering the hop’s name as the file name', async () => {
    const d = deps('/Users/me/out.wav');
    await exportTake(take, d);
    expect(d.chooseFile).toHaveBeenCalledWith('20260919 Sunday drift hoppp.wav');
  });
...
describe('exportFileName', () => {
  it('is the hop’s name, as a .wav', () => {
    expect(exportFileName('20260919 Sunday drift hoppp')).toBe('20260919 Sunday drift hoppp.wav');
  });

  it('drops characters a file name can’t have', () => {
    expect(exportFileName('20260919 Dub/Club: hoppp?')).toBe('20260919 DubClub hoppp.wav');
  });

  it('falls back to "hoppp" for a hop with no name', () => {
    expect(exportFileName('  ')).toBe('hoppp.wav');
  });
});
