...
- **App icon sources are in `project resources/Design/Hoppper app icon/`**
  (2026-09-18). The desktop icons (`src-tauri/icons/*.png`, `icon.icns`,
  `icon.ico`) are generated by `pnpm tauri icon` from the rounded
  `hoppper-icon-1024.png`. `ios/`, `android/` and the Windows `Square*Logo`
  / `StoreLogo` files come from `hoppper-icon-square-1024.png`, because
  those platforms mask the icon themselves. To regenerate: run the rounded
  one into `src-tauri/icons`, run the square one into a scratch directory,
  and copy those files over. The top bar mark and the favicon use
  `src/assets/hoppper-icon.svg`.
